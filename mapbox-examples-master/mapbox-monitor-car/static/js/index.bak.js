...
const animate = () => {
  ...
  refreshLineChart()
  const currentPnt = realTrackGeojson.features[0].geometry.coordinates[count];
  const nextPnt = realTrackGeojson.features[0].geometry.coordinates[count + 1];
  // const startPnt = trackGeojson.features[0].geometry.coordinates.slice(-1)[0];
  const startPnt = currentPnt;
  // ceshi
  const buffered = turf.buffer(turf.point(startPnt), 0.4, { units: 'kilometers' });
  // const nextbuffered = turf.buffer(turf.point(nextPnt), 0.4, { units: 'kilometers' });
  const endPnt = turf.randomPosition(turf.bbox(turf.transformScale(buffered, 1.2))); // 方大两倍来模拟接收的点位置
  var distance = turf.distance(endPnt, currentPnt, { units: 'kilometers' });

  chartData.push([moment().format('LTS'), distance * 1000])
  // console.log(endPnt);
  // end 
  ...
  // 判断它是否在buffer圆圈之内
  let isInner = turf.booleanPointInPolygon(endPnt, buffered);
  console.log(isInner)
  animatePointGeoJson.features[0].geometry.coordinates = isInner ? currentPnt : endPnt;
  ...
  trackCopy.features[0].geometry.coordinates.push(nextPnt); // 不管如何都按正确行使的轨迹进行展示
