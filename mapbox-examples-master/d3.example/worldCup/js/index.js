		...
		d3.csv("./data/WorldCupMatches.csv", function(m) {
			...
			d3.csv("./data/WorldCupRank.csv", function(r) {
				...
				yearBtns.on("click", function(d) {
					update(d);
					console.log(d)
					document.getElementById('frame1').contentWindow.postMessage({ type: 'change', data: d }, '*');
					document.getElementById('frame2').contentWindow.postMessage({ type: 'change', data: d }, '*');
				...
				playBtn.on("click", function() {
					...
					year_interval = setInterval(function() {
						...
						document.getElementById('frame1').contentWindow.postMessage({ type: 'change', data:years[year_idx] }, '*');
						document.getElementById('frame2').contentWindow.postMessage({ type: 'change', data:years[year_idx] }, '*');
						year_idx++;
	...
	*/
	...
	function colorOfCountry(d) {
		if(d.properties.name == currentCup.Winner) {
			...
			return '#8C7853';
		} else if(d.properties.name == currentCup.Country) {
			return 'red';
	...
	// 重新绘制所有图表
	drawPieBarChart();
	// drawAreaChart();  // 面积图，去掉
	// drawBubbleChart();// 气泡图，去掉
	drawRadarChart();
	drawHeatMap();
}
...
function drawBarChart(){
	console.log(cups)
	let gs = ["GoalsScored"].concat(cups.map(v => Number(v.GoalsScored)))
	let p = ["Penalty"].concat(cups.map(v => Number(v.Penalty)))
	console.log(gs)
	barChart = bb.generate({
		data: {
		  columns: [
		//   ["GoalsScored", -100, 100, 200, [-100, 0],
		//   [0, 100],
		//   [100, 200]
		//   ],
		//   ["Penalty", 100, 300, 500, [0, 100],
		//   [100, 300],
		//   [200, 500]
		//   ]
			gs , p
...
function drawLineChart(){
	console.log(cups)
	let ov = ["Overshoot"].concat(cups.map(v => Number(v.Overshoot)))
	let v = ["Overshoot"].concat(cups.map(v => Number(v.Overshoot))+1)
	console.log(ov)
	barChart = bb.generate({
		data: {
		  columns: [
