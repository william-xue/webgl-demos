...
layui.define(["jquery", "miniMenu", "element","miniTab", "miniTheme"], function (exports) {
    ...
    var miniAdmin = {
        ...
        render: function (options, callback) {
            ...
            // if(arr.length>1 && arr[1].indexOf('init_index')>-1){
            //     options.iniUrl = 'api/init.json'
            // }
            if(arr.length >1){
                let arr2 = arr[1].split('=')
                if (arr2.length > 1){
                    options.iniUrl = `api/${arr2[1]}.json`
                }
            }
            ...
            $.getJSON(options.iniUrl, function (data) {
                ...
                options.iniUrl != 'api/init.json' ? callback && callback(data) :''
            }).fail(function () {
